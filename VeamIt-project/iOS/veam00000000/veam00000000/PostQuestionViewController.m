...
{
    NSURL *url = [VeamUtil getApiUrl:@"question/post"] ;
    ...
    NSString *signature = [VeamUtil sha1:[NSString stringWithFormat:@"VEAM_%@_%d_%@",uid,socialUserId,comment]] ;
    
    NSString *params ;
    params = [NSString stringWithFormat:@"u=%@&sid=%d&q=%@&s=%@",uid,socialUserId,urlEncodedComment,signature] ;
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if([results count] >= 2){
        if([[results objectAtIndex:0] compare:@"OK" options:NSCaseInsensitiveSearch] == NSOrderedSame){
            [VeamUtil setQuestionPosted:YES] ;
            questionId = [results objectAtIndex:1] ;
            NSString *userId = [results objectAtIndex:2] ;
            NSString *userName = [results objectAtIndex:3] ;
            NSString *text = [results objectAtIndex:4] ;
