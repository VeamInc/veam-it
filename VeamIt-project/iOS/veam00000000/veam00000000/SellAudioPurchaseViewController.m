//
//  SellAudioPurchaseViewController.m
//  veam00000000
//
...
@interface SellAudioPurchaseViewController ()

@end

@implementation SellAudioPurchaseViewController
...
- (void)viewDidLoad {
    ...
    NSString *description = sellAudio.description ;
    ...
    if(!isBought){
        ...
        [label setText:sellAudio.buttonText] ;
    ...
    [self setViewName:@"AboutSellAudio/"] ;
}
...
- (void)purchaseButtonTapped
{
    ...
    if([VeamUtil isVeamConsole] || [VeamUtil isTestPurchase]){
        ...
        //NSLog(@"purchaseProductWithID : %@",sellAudio.productId) ;
        [inAppPurchaseManager purchaseProductWithID:sellAudio.productId] ;
    }
...
-(void)alertView:(UIAlertView*)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if(buttonIndex == 0){
        ...
        [VeamUtil verifySellAudioReceipt:[NSString stringWithFormat:@"TEST_TOKEN_%@",sellAudio.productId] clearIfExpired:YES forced:YES] ;
...
-(void) providePurchasedContent:(NSString *)productID
{
    ...
    isBought = YES ;
    [VeamUtil setSellAudioPurchased:YES] ;
