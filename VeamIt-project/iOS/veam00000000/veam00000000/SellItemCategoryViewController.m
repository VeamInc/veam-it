//
//  SellItemCategoryViewController.m
//  veam31000000
//
...
@interface SellItemCategoryViewController ()

@end

@implementation SellItemCategoryViewController
...
- (void)viewDidLoad
{
    ...
    //NSLog(@"SellItemCategoryViewController::viewDidLoad start") ;
    
    [self setViewName:@"SellItemCategoryList/"] ;
...
{
    indexOffset = 2 ;
    lastIndex = indexOffset + [VeamUtil getNumberOfSellItemCategories] ;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if((indexPath.row == 0) || (indexPath.row == lastIndex)){
        ...
        SellItemCategory *category = [VeamUtil getSellItemCategoryAt:index] ;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(index >= 0){
        ...
        SellItemCategory *sellItemCategory = [VeamUtil getSellItemCategoryAt:index] ;
        if(sellItemCategory != nil){
            if([VeamUtil isConnected]){
                if([sellItemCategory.kind isEqualToString:VEAM_SELL_ITEM_CATEGORY_KIND_VIDEO]){
                    VideoCategory *category = [VeamUtil getVideoCategoryForId:sellItemCategory.targetCategoryId] ;
                    if(category != nil){
                    ...
                    PdfCategory *category = [VeamUtil getPdfCategoryForId:sellItemCategory.targetCategoryId] ;
                    if(category != nil){
                        SellPdfViewController *sellPdfViewController = [[SellPdfViewController alloc] init] ;
                        [sellPdfViewController setCategoryId:[category pdfCategoryId]] ;
                        [sellPdfViewController setSubCategoryId:@"0"] ;
                        [sellPdfViewController setTitleName:[category name]] ;
                        [self.navigationController pushViewController:sellPdfViewController animated:YES] ;
                    }
                } else if([sellItemCategory.kind isEqualToString:VEAM_SELL_ITEM_CATEGORY_KIND_AUDIO]){
                    //NSLog(@"category audio") ;
                    AudioCategory *category = [VeamUtil getAudioCategoryForId:sellItemCategory.targetCategoryId] ;
                    if(category != nil){
                        SellAudioViewController *sellAudioViewController = [[SellAudioViewController alloc] init] ;
                        [sellAudioViewController setCategoryId:[category audioCategoryId]] ;
                        [sellAudioViewController setSubCategoryId:@"0"] ;
                        [sellAudioViewController setTitleName:[category name]] ;
                        [self.navigationController pushViewController:sellAudioViewController animated:YES] ;
                    }
                }
            } else {
