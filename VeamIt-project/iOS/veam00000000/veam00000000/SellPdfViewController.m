...
#import "SellPdfPurchaseViewController.h"
#import "SellPdf.h"
#import <QuartzCore/QuartzCore.h>

@interface SellPdfViewController ()

@end

@implementation SellPdfViewController
...
- (void)viewDidLoad
{
    ...
    [self setViewName:[NSString stringWithFormat:@"SellPdfList/"]] ;
...
{
    indexOffset = 1 ;
    sellPdfs = [VeamUtil getSellPdfsForCategory:categoryId] ;
    //NSLog(@"numberOfSellPdfs=%d categoryId=%@",[sellPdfs count],categoryId) ;
    lastIndex = indexOffset + [sellPdfs count] ;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if((indexPath.row == 0) || (indexPath.row == lastIndex)){
        ...
        Pdf *pdf = [VeamUtil getPdfForId:[sellPdf pdfId]] ;
        ...
        if([sellPdf isBought]){
            ...
            NSString *priceText = sellPdf.priceText ;
        ...
        NSString *thumbnailUrl = [pdf imageUrl] ;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(index >= 0){
        SellPdf *sellPdf = [sellPdfs objectAtIndex:index] ;
        if(sellPdf != nil){
            //NSLog(@"sellPdf tapped : %@ %@",[sellPdf sellPdfId],[sellPdf pdfId]) ;
            if([sellPdf isBought]){
                Pdf *pdf = [VeamUtil getPdfForId:sellPdf.pdfId] ;
                // play
                [self playPdf:pdf] ;
                ...
                // purchase
                SellPdfPurchaseViewController *sellPdfPurchaseViewController = [[SellPdfPurchaseViewController alloc] init] ;
                [sellPdfPurchaseViewController setTitleName:NSLocalizedString(@"about_pdf", nil)] ;
                [sellPdfPurchaseViewController setSellPdf:sellPdf] ;
                [self.navigationController pushViewController:sellPdfPurchaseViewController animated:YES] ;
                
                //[self purchaseButtonTapped:sellPdf.productId] ;
            }
        }
    }
}
...
{
    NSString *pdfUrl = [VeamUtil getPdfUrl:pdf.pdfId] ;
    //NSLog(@"pdfUrl=%@",pdfUrl) ;
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if([VeamUtil getSellPdfPurchased]){
        [VeamUtil setSellPdfPurchased:NO] ;
        [self performSelectorOnMainThread:@selector(reloadList) withObject:nil waitUntilDone:NO] ;
    }
}
