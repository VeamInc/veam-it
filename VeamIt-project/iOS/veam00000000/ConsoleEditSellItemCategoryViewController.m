//
//  ConsoleEditSellItemCategoryViewController.m
//  veam00000000
//
...
@interface ConsoleEditSellItemCategoryViewController ()

@end

@implementation ConsoleEditSellItemCategoryViewController

@synthesize sellItemCategory ;
...
- (void)reloadValues
{
    ...
    if(sellItemCategory != nil){
        title = [[ConsoleUtil getConsoleContents] getCategoryTitleForSellItemCategory:sellItemCategory] ;
        kind = sellItemCategory.kind ;
    }
...
- (void)contentsDidUpdate:(NSNotification*)notification
{
    ...
    if(notification.userInfo != nil){
        ...
        if([actionName isEqualToString:@"CONTENT_POST_DONE"]){
            ...
            if([apiName isEqualToString:@"sellitem/setcategory"]){
                ...
                if([results count] >= 5){
                    ...
                    if([code isEqualToString:@"OK"]){
                        NSString *sellItemCategoryId = [results objectAtIndex:1] ;
                        NSString *kind = [results objectAtIndex:2] ;
                        NSString *targetCategoryId = [results objectAtIndex:3] ;
                        NSString *name = [results objectAtIndex:4] ;
                        [[ConsoleUtil getConsoleContents] setSellItemTarget:kind targetCategoryId:targetCategoryId name:name] ;
...
- (BOOL)isValueChanged
{
    ...
    if(sellItemCategory == nil){
        ...
        if(
           ![title isEqualToString:[[ConsoleUtil getConsoleContents] getCategoryTitleForSellItemCategory:sellItemCategory]] ||
           ![kind isEqualToString:sellItemCategory.kind]
           ){
            //NSLog(@"modified") ;
...
- (BOOL)saveValues
{
    ...
    if(sellItemCategory == nil){
        sellItemCategory = [[SellItemCategory alloc] init] ;
    }
    //NSLog(@"name=%@ url=%@",title,url) ;
    [sellItemCategory setKind:kind] ;
    [[ConsoleUtil getConsoleContents] setSellItemCategory:sellItemCategory title:title] ;
    [self showProgress] ;
