//
//  ConsoleSellAudioTableViewCell.m
//  veam00000000
//
...
#define CONSOLE_SELL_AUDIO_CELL_HEIGHT       88
#define CONSOLE_SELL_AUDIO_CELL_LEFTMARGIN   10
#define CONSOLE_SELL_AUDIO_CELL_RIGHTMARGIN  6


@implementation ConsoleSellAudioTableViewCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier sellAudio:(SellAudio *)sellAudio isLast:(BOOL)isLast
{
    ...
    if (self) {
        ...
        if([sellAudio.status isEqualToString:VEAM_SELL_AUDIO_STATUS_WAITING]){
        ...
        CGFloat currentX = [VeamUtil getScreenWidth] - CONSOLE_SELL_AUDIO_CELL_RIGHTMARGIN - rightImage.size.width/2 ;
        
        self.rightImageView  = [[UIImageView alloc] initWithFrame:CGRectMake(currentX, (CONSOLE_SELL_AUDIO_CELL_HEIGHT-rightImage.size.height/2)/2, rightImage.size.width/2, rightImage.size.height/2)] ;
        self.rightImageView.image = rightImage ;
        [self.contentView addSubview:self.rightImageView] ;
        
        
        /*
         self.moveImageView  = [[UIImageView alloc] initWithFrame:CGRectMake(CONSOLE_SELL_AUDIO_CELL_LEFTMARGIN, (CONSOLE_SELL_AUDIO_CELL_HEIGHT-moveImage.size.height/2)/2, moveImage.size.width/2, moveImage.size.height/2)] ;
        ...
        CGFloat thumbnailY = (CONSOLE_SELL_AUDIO_CELL_HEIGHT-thumbnailHeight)/2 ;
        self.thumbnailImageView  = [[UIImageView alloc] initWithFrame:CGRectMake(CONSOLE_SELL_AUDIO_CELL_LEFTMARGIN, thumbnailY, thumbnailWidth, thumbnailHeight)] ;
        ...
        self.titleLabel.text = [[ConsoleUtil getConsoleContents] getAudioForId:sellAudio.audioId].title ;
        ...
        self.priceLabel.text = sellAudio.priceText ;
        ...
        //NSLog(@"sellAudio.status=%@",sellAudio.status) ;
        if([sellAudio.status isEqualToString:VEAM_SELL_AUDIO_STATUS_READY]){
            ...
            //self.statusLabel.text = sellAudio.statusText ;
            NSAttributedString *astr = [[NSAttributedString alloc]
                                        initWithString:sellAudio.statusText
                                        attributes:@{
            ...
            if([sellAudio.status isEqualToString:VEAM_SELL_AUDIO_STATUS_PREPARING] ||
               [sellAudio.status isEqualToString:VEAM_SELL_AUDIO_STATUS_SUBMITTING]){
                [self blinkLabel:self.statusLabel] ;
            }
            
            if([sellAudio.status isEqualToString:VEAM_SELL_AUDIO_STATUS_SUBMITTING]){
        ...
        if(isLast){
            self.separatorView = [[UIView alloc] initWithFrame:CGRectMake(0, CONSOLE_SELL_AUDIO_CELL_HEIGHT-1, [VeamUtil getScreenWidth], 0.5)] ;
        } else {
            self.separatorView = [[UIView alloc] initWithFrame:CGRectMake(CONSOLE_SELL_AUDIO_CELL_LEFTMARGIN, CONSOLE_SELL_AUDIO_CELL_HEIGHT-1, [VeamUtil getScreenWidth], 0.5)] ;
        }
        self.separatorView.backgroundColor = [UIColor blackColor] ;
        [self.contentView addSubview:self.separatorView] ;
        
        self.deleteTapView = [[UIView alloc] initWithFrame:CGRectMake([VeamUtil getScreenWidth]-CONSOLE_SELL_AUDIO_CELL_HEIGHT, 0, CONSOLE_SELL_AUDIO_CELL_HEIGHT, CONSOLE_SELL_AUDIO_CELL_HEIGHT)] ;
...
{
    return CONSOLE_SELL_AUDIO_CELL_HEIGHT ;
}
