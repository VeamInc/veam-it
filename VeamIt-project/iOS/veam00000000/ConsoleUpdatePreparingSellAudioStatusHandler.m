//
//  ConsoleUpdatePreparingSellAudioStatusHandler.m
//  veam00000000
//
...
@implementation ConsoleUpdatePreparingSellAudioStatusHandler
    ...
    if([results count] >= 2){
        ...
        if([result isEqualToString:@"OK"]){
            ...
            for(int index = 0 ; index < numberOfElements ; index++){
                ...
                if([values count] >= 3){
                    NSString *sellAudioId = [values objectAtIndex:0] ;
                    NSString *status = [values objectAtIndex:1] ;
                    NSString *statusText = [values objectAtIndex:2] ;
                    SellAudio *sellAudio = [contents getSellAudioForId:sellAudioId] ;
                    if(sellAudio != nil){
