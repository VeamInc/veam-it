//
//  ConsoleEditYoutubeCategoryViewController.m
//  veam00000000
//
...
@interface ConsoleEditYoutubeCategoryViewController ()

@end

@implementation ConsoleEditYoutubeCategoryViewController
...
- (CGFloat)addContents:(CGFloat)y
{
    ...
    embedUrlInputBarView = [self addTextInputBar:@"Embed URL" y:currentY fullBottomLine:YES selector:nil tag:CONSOLE_VIEW_EMBED_URL] ;
...
- (void)reloadValues
{
    ...
    NSString *embed = @"" ;
    NSString *embedUrl = @"" ;
    if(youtubeCategory != nil){
        title = youtubeCategory.name ;
        embed = youtubeCategory.embed ;
        embedUrl = youtubeCategory.embedUrl ;
    }
    [titleInputBarView setInputValue:title] ;
    [embedFlagInputBarView setInputValue:[embed isEqualToString:@"1"]] ;
    [embedUrlInputBarView setInputValue:embedUrl] ;
}
...
- (void)didTapRightText
{
    ...
    if(youtubeCategory == nil){
        youtubeCategory = [[YoutubeCategory alloc] init] ;
    }
    //NSLog(@"name=%@",[titleInputBarView getInputValue]) ;
    [youtubeCategory setName:[titleInputBarView getInputValue]] ;
    [youtubeCategory setEmbed:[embedFlagInputBarView getInputValue]?@"1":@"0"] ;
    [youtubeCategory setEmbedUrl:[embedUrlInputBarView getInputValue]] ;
    [[ConsoleUtil getConsoleContents] setYoutubeCategory:youtubeCategory] ;
