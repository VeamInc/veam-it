...
#define CONSOLE_SELL_PDF_CELL_HEIGHT       88
#define CONSOLE_SELL_PDF_CELL_LEFTMARGIN   10
#define CONSOLE_SELL_PDF_CELL_RIGHTMARGIN  6


@implementation ConsoleSellPdfTableViewCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier sellPdf:(SellPdf *)sellPdf isLast:(BOOL)isLast
{
    ...
    if (self) {
        ...
        if([sellPdf.status isEqualToString:VEAM_SELL_PDF_STATUS_WAITING]){
        ...
        CGFloat currentX = [VeamUtil getScreenWidth] - CONSOLE_SELL_PDF_CELL_RIGHTMARGIN - rightImage.size.width/2 ;
        
        self.rightImageView  = [[UIImageView alloc] initWithFrame:CGRectMake(currentX, (CONSOLE_SELL_PDF_CELL_HEIGHT-rightImage.size.height/2)/2, rightImage.size.width/2, rightImage.size.height/2)] ;
        self.rightImageView.image = rightImage ;
        [self.contentView addSubview:self.rightImageView] ;
        
        
        /*
         self.moveImageView  = [[UIImageView alloc] initWithFrame:CGRectMake(CONSOLE_SELL_PDF_CELL_LEFTMARGIN, (CONSOLE_SELL_PDF_CELL_HEIGHT-moveImage.size.height/2)/2, moveImage.size.width/2, moveImage.size.height/2)] ;
        ...
        CGFloat thumbnailY = (CONSOLE_SELL_PDF_CELL_HEIGHT-thumbnailHeight)/2 ;
        self.thumbnailImageView  = [[UIImageView alloc] initWithFrame:CGRectMake(CONSOLE_SELL_PDF_CELL_LEFTMARGIN, thumbnailY, thumbnailWidth, thumbnailHeight)] ;
        ...
        self.titleLabel.text = [[ConsoleUtil getConsoleContents] getPdfForId:sellPdf.pdfId].title ;
        ...
        self.priceLabel.text = sellPdf.priceText ;
        ...
        //NSLog(@"sellPdf.status=%@",sellPdf.status) ;
        if([sellPdf.status isEqualToString:VEAM_SELL_PDF_STATUS_READY]){
            ...
            //self.statusLabel.text = sellPdf.statusText ;
            NSAttributedString *astr = [[NSAttributedString alloc]
                                        initWithString:sellPdf.statusText
                                        attributes:@{
            ...
            if([sellPdf.status isEqualToString:VEAM_SELL_PDF_STATUS_PREPARING] ||
               [sellPdf.status isEqualToString:VEAM_SELL_PDF_STATUS_SUBMITTING]){
                [self blinkLabel:self.statusLabel] ;
            }
            
            if([sellPdf.status isEqualToString:VEAM_SELL_PDF_STATUS_SUBMITTING]){
        ...
        if(isLast){
            self.separatorView = [[UIView alloc] initWithFrame:CGRectMake(0, CONSOLE_SELL_PDF_CELL_HEIGHT-1, [VeamUtil getScreenWidth], 0.5)] ;
        } else {
            self.separatorView = [[UIView alloc] initWithFrame:CGRectMake(CONSOLE_SELL_PDF_CELL_LEFTMARGIN, CONSOLE_SELL_PDF_CELL_HEIGHT-1, [VeamUtil getScreenWidth], 0.5)] ;
        }
        self.separatorView.backgroundColor = [UIColor blackColor] ;
        [self.contentView addSubview:self.separatorView] ;
        

        self.deleteTapView = [[UIView alloc] initWithFrame:CGRectMake([VeamUtil getScreenWidth]-CONSOLE_SELL_PDF_CELL_HEIGHT, 0, CONSOLE_SELL_PDF_CELL_HEIGHT, CONSOLE_SELL_PDF_CELL_HEIGHT)] ;
...
{
    return CONSOLE_SELL_PDF_CELL_HEIGHT ;
}
