...
#		print("++++++++++++++++++ fps = %s\n",$fps) ;
		...
		$width = $height ;
		if($android){
			$crop = sprintf("crop=%d:%d:0:0",$width,$height) ;
		} else {
			$crop = sprintf("crop=%d:%d:%d:0",$width,$height,int($height/12)) ;
		}
		...
		if($compValue == $width){
			...
			if($compValue > $width){
				$padding = sprintf("pad=%d:%d:%d:0:black ",$compValue,$height,int(($compValue-$width)/2)) ;
				...
				$padding = sprintf("pad=%d:%d:0:%d:black ",$width,int($width*9/16),int(($width*9/16-$height)/2)) ;
		...
		$command = sprintf("ffmpeg -y -i %s -vcodec libx264 -b 256k -bt 256k -g 33 -qscale 1 -qmin 2 -qmax 37 -qcomp 0.7 -qdiff 4 -subq 6 -me_range 16 -i_qfactor 0.714286 -aspect 16:9 -s %dx%d -r %s -vf %s,transpose=1,%s -metadata:s:v:0 rotate=0 -an %s.m4v",$fileName,$newWidth,$newHeight,$fps,$crop,$padding,$encodedFilePrefix) ;
