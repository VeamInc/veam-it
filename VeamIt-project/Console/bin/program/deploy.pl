...
if($programId && $dest){
	...
	$sqlFile = sprintf('sql/%s_%s_%04d%02d%02d%02d%02d%02d.txt',$toDb,$programId,$year+1900,$month+1,$mday,$hour,$min,$sec) ;
	...
	$command = sprintf('mysqldump -u %s -p%s -h __DB_HOST_NAME__ -t --opt %s %s "-wid=%s"',$fromDb,$fromPass,$fromDb,$tableName,$programId) ;
	$result = `$command` ;
	#$result =~ s/--.*?\n//g ;
	$result =~ s/\/\*.*?\*\///g ;
	@lines = split(/\n/,$result) ;
