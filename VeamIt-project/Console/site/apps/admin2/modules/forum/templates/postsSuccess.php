	...
	<div class="widget-title"><h5 class="widgetheading">Posts</h5></div>
		...
		<div class="element-select">
			...
			<select onChange="selectJump(this)">
				<option value="<?php echo url_for(sprintf('%s/%s?a=%d&f=0&p=1&so=%d',$sf_context->getModuleName(),$sf_context->getActionName(),0,$sortKind), false) ?>" <?php if(!$appId) echo 'selected' ; ?>>All</option>
				<?php foreach($allApps as $workApp): ?>
				<?php if($workApp->getStatus() == 0):?>
				<option value="<?php echo url_for(sprintf('%s/%s?a=%d&f=0&p=1&so=%d',$sf_context->getModuleName(),$sf_context->getActionName(),$workApp->getId(),$sortKind), false) ?>" <?php if($appId == $workApp->getId()) echo 'selected' ; ?>><?php echo $workApp->getName() ?></option>
				<?php endif ?>
				<?php endforeach ?>
			</select>
			Forum :
		...
		<div id="pagination">
			...
			<?php if($page != 1): ?>
			<?php echo AdminTools::link_to("<<", "forum/posts", array("class"=>"inactive", "query_string"=>sprintf("a=%s&f=%d&p=1&so=%d",$appId,$forumId,$sortKind))) ; ?>
			<?php echo AdminTools::link_to("<", "forum/posts", array("class"=>"inactive", "query_string"=>sprintf("a=%s&f=%d&p=%d&so=%d",$appId,$forumId,$page-1,$sortKind))) ; ?>
			<?php endif ?>
			<?php 
				for($workPage = $startPage ; $workPage <= $endPage ; $workPage++){ 
					if($workPage == $page){
						...
						echo AdminTools::link_to($workPage, "forum/posts", array("class"=>"inactive", "query_string"=>sprintf("a=%s&f=%d&p=%d&so=%d",$appId,$forumId,$workPage,$sortKind))) ;
					}
				}
			?>
			<?php if($page != $lastPage): ?>
			<?php echo AdminTools::link_to(">", "forum/posts", array("class"=>"inactive", "query_string"=>sprintf("a=%s&f=%d&p=%d&so=%d",$appId,$forumId,$page+1,$sortKind) )) ; ?>
			<?php echo AdminTools::link_to(">>", "forum/posts", array("class"=>"inactive", "query_string"=>sprintf("a=%s&f=%d&p=%d&so=%d",$appId,$forumId,$lastPage,$sortKind) )) ; ?>
			<?php endif ?>
		</div>
		<!-- End pagination-->
