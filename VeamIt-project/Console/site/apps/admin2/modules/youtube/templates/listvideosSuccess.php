...
<div class="span9 column">
	<div class="widget-title"><h5 class="widgetheading">YouTube Videos</h5></div>
		...
		<div class="element-select">
			...
			Category :
			<select onChange="selectJump(this)">
				<option value="<?php echo url_for(sprintf('%s/%s?a=%s&c=0&p=1&so=%d',$sf_context->getModuleName(),$sf_context->getActionName(),$appId,$sortKind), false) ?>" <?php if(!$categoryId) echo 'selected' ; ?>>All</option>
				<?php foreach($categoriesForList as $category): ?>
				<option value="<?php echo url_for(sprintf('%s/%s?a=%s&c=%d&p=1&so=%d',$sf_context->getModuleName(),$sf_context->getActionName(),$appId,$category->getId(),$sortKind), false) ?>" <?php if($categoryId == $category->getId()) echo 'selected' ; ?>><?php echo AdminTools::unescapeName($category->getName()) ?></option>
				<?php endforeach ?>
			</select>
			...
			<select onChange="selectJump(this)">
				...
				<option value="<?php echo url_for(sprintf('%s/%s?a=%s&c=%d&p=1&so=%d',$sf_context->getModuleName(),$sf_context->getActionName(),$appId,$categoryId,$workSortKind), false) ?>" <?php if($workSortKind == $sortKind) echo 'selected' ; ?>><?php echo $sortName ?></option>
				<?php endforeach ?>
			</select>
			</span>
			<p style="clear:both" />
		...
		<div id="pagination">
			...
			<?php if($page != 1): ?>
			<?php echo AdminTools::link_to("<<", "youtube/listvideos", array("class"=>"inactive", "query_string"=>sprintf("a=%s&c=%d&p=1&so=%d",$appId,$categoryId,$sortKind))) ; ?>
			<?php echo AdminTools::link_to("<", "youtube/listvideos", array("class"=>"inactive", "query_string"=>sprintf("a=%s&c=%d&p=%d&so=%d",$appId,$categoryId,$page-1,$sortKind))) ; ?>
			<?php endif ?>
			<?php 
				for($workPage = $startPage ; $workPage <= $endPage ; $workPage++){ 
					if($workPage == $page){
						...
						echo AdminTools::link_to($workPage, "youtube/listvideos", array("class"=>"inactive", "query_string"=>sprintf("a=%s&c=%d&p=%d&so=%d",$appId,$categoryId,$workPage,$sortKind))) ;
					}
				}
			?>
			<?php if($page != $lastPage): ?>
			<?php echo AdminTools::link_to(">", "youtube/listvideos", array("class"=>"inactive", "query_string"=>sprintf("a=%s&c=%d&p=%d&so=%d",$appId,$categoryId,$page+1,$sortKind) )) ; ?>
			<?php echo AdminTools::link_to(">>", "youtube/listvideos", array("class"=>"inactive", "query_string"=>sprintf("a=%s&c=%d&p=%d&so=%d",$appId,$categoryId,$lastPage,$sortKind) )) ; ?>
			<?php endif ?>
		</div>
		<!-- End pagination-->
