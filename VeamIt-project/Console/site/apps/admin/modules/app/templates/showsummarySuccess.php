			...
			<div class="accordion-group">
				...
				<div id="collapse-as-1" class="accordion-body collapse">
					<div class="accordion-inner">
						...
						<?php endforeach ?>
						<?php echo link_to("+ALL", "subscription/listcontents", array("class"=>"pull-right", "query_string"=>sprintf("a=%d&p=1",$app->getId()))) ; ?>
						<p style="clear:both" />
						<?php else: ?>
						...
						<table class="table table-striped">
							<tbody>
								...
								<tr>
									...
									<td style="width:15px">
										<?php echo link_to('<i class="icon-folder-close"></i>', "youtube/listvideos", array("class"=>"contact-more","query_string"=>sprintf("a=%d&c=%d&p=1",$app->getId(),$youtubeCategory->getId()))) ; ?>
									</td>
								</tr>
								<?php } ?>
							</tbody>
						...
						<table class="table table-striped">
							<tbody>
								...
								<tr>
									...
									<td style="width:15px">
										<?php echo link_to('<i class="icon-folder-close"></i>', "forum/posts", array("class"=>"contact-more","query_string"=>sprintf("a=%d&f=%d&p=1",$app->getId(),$forum->getId()))) ; ?>
									</td>
								</tr>
								<?php } ?>
							</tbody>
			...
			<div class="accordion-group">
				...
				<div id="collapse-as-4" class="accordion-body collapse">
					<div class="accordion-inner">
								...
								<div class="media-content">
									...
									<div class="bottom-article-no-margin">
										<ul class="meta-post">
											...
											<?php if(!$removed): ?>
											<li><i class="icon-trash"></i><?php echo link_to("Remove this post", "forum/deletepost", array("onclick"=>'return confirm("Remove this post?")',"query_string"=>sprintf("pid=%d&a=%s&p=%s",$picture->getId(),$app->getId(),$page))) ; ?></li>
											<?php endif ?>
										</ul>
									</div>
								</div>
						...
						<?php echo link_to("+ALL", "forum/reports", array("class"=>"pull-right", "query_string"=>sprintf("a=%d&p=1",$app->getId()))) ; ?>
						<p style="clear:both" />
			...
			<div class="accordion-group">
				...
				<div id="collapse-as-6" class="accordion-body collapse">
					<div class="accordion-inner">
						...
						<h6>Rating</h6>
						<?php if(count($appRatingQuestions) > 0): ?>
						<table class="table table-striped">
							<tbody>
								<?php 
									$count = 0 ;
									foreach($appRatingQuestions as $appRatingQuestion){
										$count++ ;
										if(isset($appRatingAnswerMapForQuestion[$appRatingQuestion->getId()])){
											$appRatingAnswer = $appRatingAnswerMapForQuestion[$appRatingQuestion->getId()] ;
										} else {
											$appRatingAnswer = "" ;
										}
								?>
								<tr>
									<td><?php echo $count ?></td>
									<td><?php echo $appRatingQuestion->getQuestion() ?></td>
									<td>
										<?php if($appRatingAnswer): ?>
										<?php echo $appRatingAnswer->getAnswer() ?>
										<?php else: ?>
										<span class="media-opacity40">Not Answered</span>
										<?php endif ?>
									</td>
								</tr>
								<?php } ?>
