...
<div class="span9 column">
	<div class="widget-title"><h5 class="widgetheading">Released Apps</h5></div>
		...
		<div id="pagination">
			...
			<?php if($page != 1): ?>
			<?php echo AdminTools::link_to("<<", "app/listreleased", array("class"=>"inactive", "query_string"=>sprintf("p=1&so=%d",$sortKind))) ; ?>
			<?php echo AdminTools::link_to("<", "app/listreleased", array("class"=>"inactive", "query_string"=>sprintf("p=%d&so=%d",$page-1,$sortKind))) ; ?>
			<?php endif ?>
			<?php 
				for($workPage = $startPage ; $workPage <= $endPage ; $workPage++){ 
					if($workPage == $page){
						...
						echo AdminTools::link_to($workPage, "app/listreleased", array("class"=>"inactive", "query_string"=>sprintf("p=%d&so=%d",$workPage,$sortKind))) ;
					}
				}
			?>
			<?php if($page != $lastPage): ?>
			<?php echo AdminTools::link_to(">", "app/listreleased", array("class"=>"inactive", "query_string"=>sprintf("p=%d&so=%d",$page+1,$sortKind) )) ; ?>
			<?php echo AdminTools::link_to(">>", "app/listreleased", array("class"=>"inactive", "query_string"=>sprintf("p=%d&so=%d",$lastPage,$sortKind) )) ; ?>
			<?php endif ?>
		</div>
		<!-- End pagination-->
