...
<div class="span9 column">
	<div class="widget-title"><h5 class="widgetheading">Video</h5></div>
		...
		<?php if(count($sellVideos) > 0): ?>
		<?php 
			foreach($sellVideos as $sellVideo): 
				$app = $appMap[$sellVideo->getAppId()] ;
				if($app){
					$appName = $app->getName() ;
				} else {
					$appName = "" ;
				...
				$currentStatus = $sellVideo->getStatus() ;
		?>
		<div class="media">
			...
			<div class="media-body">
				<div class="media-content">
					...
					<div class="bottom-article-no-margin">
						<ul class="meta-post">
							<li><i class="icon-calendar"></i><?php echo $sellVideo->getCreatedAt() ?></li>
							<li><i class="icon-folder-open"></i><?php echo $appName ?></li>
							<li>Status : <?php echo $statuses[$currentStatus] ?></li>
							<?php if($currentStatus == 3): ?>
							<li><i class="icon-info-sign"></i><a href="/admin.php/help/registerppc?k=1&i=<?php echo $sellVideo->getId() ?>" >Register to app store</a></li>
							<?php endif ?>
						</ul>
					</div>
				</div>
		...
		<div id="pagination">
			...
			<?php if($page != 1): ?>
			<?php echo AdminTools::link_to("<<", "paypercontent/listvideo", array("class"=>"inactive", "query_string"=>sprintf("a=%s&p=1&so=%d&s=%d",$appId,$sortKind,$status))) ; ?>
			<?php echo AdminTools::link_to("<", "paypercontent/listvideo", array("class"=>"inactive", "query_string"=>sprintf("a=%s&p=%d&so=%d&s=%d",$appId,$page-1,$sortKind,$status))) ; ?>
			<?php endif ?>
			<?php 
				for($workPage = $startPage ; $workPage <= $endPage ; $workPage++){ 
					if($workPage == $page){
						...
						echo AdminTools::link_to($workPage, "paypercontent/listvideo", array("class"=>"inactive", "query_string"=>sprintf("a=%s&p=%d&so=%d&s=%d",$appId,$workPage,$sortKind,$status))) ;
					}
				}
			?>
			<?php if($page != $lastPage): ?>
			<?php echo AdminTools::link_to(">", "paypercontent/listvideo", array("class"=>"inactive", "query_string"=>sprintf("a=%s&p=%d&so=%d&s=%d",$appId,$page+1,$sortKind,$status) )) ; ?>
			<?php echo AdminTools::link_to(">>", "paypercontent/listvideo", array("class"=>"inactive", "query_string"=>sprintf("a=%s&p=%d&so=%d&s=%d",$appId,$lastPage,$sortKind,$status) )) ; ?>
			<?php endif ?>
		</div>
		<!-- End pagination-->
