...
class completeAppprocessTask extends sfBaseTask
{
  protected function configure()
  {
    ...
    $this->namespace        = '';
    $this->name             = 'completeAppprocess';
    $this->briefDescription = '';
    $this->detailedDescription = <<<EOF
The [completeAppprocess|INFO] task does things.
Call it with:

  [php symfony completeAppprocess|INFO]
EOF;
  }
  ...
  protected function execute($arguments = array(), $options = array())
  {
	...
	if($remoteCommand){
		...
		if($workingRemoteCommand){
			...
			$appId = $remoteCommand->getAppId() ;
			$params = explode(",",$remoteCommand->getParams()) ;
			$appProcessId = $params[0] ;
			$processResult = $params[1] ;

			AdminTools::completeProcess($appId,$appProcessId,$processResult,"-",$databaseManager) ;
