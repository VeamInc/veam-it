...
class deployAppcontentsTask extends sfBaseTask
{
  protected function configure()
  {
    ...
    $this->namespace        = '';
    $this->name             = 'deployAppcontents';
    $this->briefDescription = '';
    $this->detailedDescription = <<<EOF
The [deployAppcontents|INFO] task does things.
Call it with:

  [php symfony deployAppcontents|INFO]
EOF;
  }
  ...
  protected function execute($arguments = array(), $options = array())
  {
	...
	if($remoteCommand){
		...
		if($workingRemoteCommand){
			...
			$appId = $remoteCommand->getAppId() ;
			AdminTools::deployAppContents($appId,$databaseManager) ;
