...
class updateYoutubelistTask extends sfBaseTask
{
  protected function configure()
  {
    ...
    $this->namespace        = '';
    $this->name             = 'updateYoutubelist';
    $this->briefDescription = '';
    $this->detailedDescription = <<<EOF
The [updateYoutubelist|INFO] task does things.
Call it with:

  [php symfony updateYoutubelist|INFO]
EOF;
  }
...
print("validChannelId $channelId\n") ;
	...
	public function updateOneApp($youtubeUser)
	{
		...
		foreach($categoriesToBeUpdated as $category){
			...
			if($playlistId == $uploadsPlaylistId){
				...
				/*
				uasort(&$videos, 'playlistItemComp');

				print("--- new video after sorting ---\n") ;
				foreach($videos as $video){
...
function playlistItemComp($a,$b) {
	$time1 = strtotime($a->{'snippet'}->{'publishedAt'}) ;
	$time2 = strtotime($b->{'snippet'}->{'publishedAt'}) ;
    if ($time1 == $time2) {
        return 0;
    }
