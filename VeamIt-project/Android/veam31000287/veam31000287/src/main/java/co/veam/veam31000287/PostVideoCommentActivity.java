...
public class PostVideoCommentActivity extends VeamActivity implements OnClickListener {
	// カメラインスタンス
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		this.pageName = String.format("PostVideoComment/%s",videoId) ;
	...
	public void operatePostComment(){
		...
		if(VeamUtil.isEmpty(comment)){
			...
			PostVideoCommentTask postCommentTask = new PostVideoCommentTask(this,videoId,comment) ;
	...
	public void onPostCommentFinished(VideoCommentObject videoCommentObject){
		//VeamUtil.log("debug","onPostCommentFinished") ;
		postProgressBar.setVisibility(View.GONE) ;
		postTextView.setVisibility(View.VISIBLE); 
		if(videoCommentObject != null){
			//VeamUtil.log("debug","videoCommentObject != null") ;
			this.setResult(1) ;
			this.finish() ;
		} else {
