 ...
 */
public class ConsoleMessageAdapter extends LoadMoreAdapter {
    ...
    private ConsoleMessageAdapterActivityInterface messageActivity ;
    private Context context ;

    ArrayList<ConsoleMessageObject> messages ;
    ...
    public ConsoleMessageAdapter(ConsoleMessageAdapterActivityInterface activity,Context context,ConsoleMessageXml messageXml,int width,int topMargin,float scaledDensity)
    {
    ...
    public void setMessages(ConsoleMessageXml messageXml){
        if(messageXml != null){
            this.messages = messageXml.getMessages() ;
            if(VeamUtil.isEmpty(appCreatorName)){
                appCreatorName = messageXml.getAppCreatorName() ;
            }
            if(VeamUtil.isEmpty(mcnName)){
                mcnName = messageXml.getMcnName() ;
            }
        }
        updating = false ;
    }
    ...
    public boolean hasMessage(String messageId){
        ...
        if(messages != null){
            ...
            for(int index = 0 ; index < count ; index++){
                ConsoleMessageObject message = messages.get(index) ;
    ...
    public void addMessages(ConsoleMessageXml messageXml){
        loadingMore = false ;
        if(messages == null){
            messages = new ArrayList<ConsoleMessageObject>() ;
        }
        ArrayList<ConsoleMessageObject> workMessages = messageXml.getMessages() ;
        int count = workMessages.size() ;
        for(int index = 0 ; index < count ; index++){
            ConsoleMessageObject message = workMessages.get(index) ;
    ...
    public Object getItem(int position) {
        ConsoleMessageObject retValue = null ;
        if(position > 0){
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        ConsoleMessageObject messageObject = (ConsoleMessageObject)this.getItem(position) ;
        ...
        if(messageObject != null){
            VeamUtil.log("debug", "direction=" + messageObject.getDirection()) ;
            isMyMessage = messageObject.getDirection().equals("1");
        }

        if(position > 0){
            if(messageObject != null){
                if(isMyMessage){
                    title = String.format("%s   ", appCreatorName) ;
                } else {
                    title = mcnName ;
                }
            }
		...
		*/
        ...
        if(view == null){
            ...
            userIconImageView.setScaleType(ImageView.ScaleType.CENTER_CROP) ;
        ...
        if(isMyMessage){
            ...
            textTextView.setBackgroundResource(R.drawable.textline_gray) ;
            imageView.setImageResource(R.drawable.pick_gray) ;
    ...
    public String getDisplayDate(int position){
        ...
        ConsoleMessageObject messageObject = (ConsoleMessageObject)getItem(position) ;
        if(messageObject != null){
            String currentDate = VeamUtil.getMessageDateString(messageObject.getCreatedAt()) ;
            ConsoleMessageObject previousMessageObject = (ConsoleMessageObject)getItem(position-1) ;
            String previousYear = "" ;
    ...
    public interface  ConsoleMessageAdapterActivityInterface {
