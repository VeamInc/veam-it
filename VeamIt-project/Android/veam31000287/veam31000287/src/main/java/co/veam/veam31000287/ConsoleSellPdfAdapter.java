 ...
 */
public class ConsoleSellPdfAdapter extends ConsoleBaseAdapter {
    ...
    public ConsoleSellPdfAdapter(ConsoleActivity consoleActivity,String pdfCategoryId)
    {
    ...
    public int getCount() {
        ...
        if(consoleContents != null){
            retValue = consoleContents.getNumberOfSellPdfsForPdfCategory(pdfCategoryId) ;
        }
        retValue++ ; // + Add New
    ...
    public Object getItem(int position) {
        ...
        if(position < cellCount-1) {
            ConsoleContents consoleContents = ConsoleUtil.getConsoleContents();
            if (consoleContents != null) {
                SellPdfObject sellPdfObject = consoleContents.getSellPdfForPdfCategory(pdfCategoryId, position, 0) ;
                item = (Object)sellPdfObject ;
            }
        } else if(position == cellCount-1){
            ConsoleAdapterElement adapterElement = new ConsoleAdapterElement(0, ConsoleAdapterElement.KIND_TITLE_ONLY, ConsoleBaseAdapter.COLOR_TYPE_LEFT_RED, "+ " + consoleActivity.getString(R.string.add_new_pdf), "", null);
    ...
    */
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(position == cellCount-1){
            ...
            SellPdfObject sellPdf = (SellPdfObject)this.getItem(position) ;
            ...
            String title = "" ;
            String price = sellPdf.getPriceText() ;
            ConsoleContents consoleContents = ConsoleUtil.getConsoleContents() ;
            if(consoleContents != null){
                pdf = consoleContents.getPdfForId(sellPdf.getPdfId()) ;
                if(pdf != null) {
            ...
            VeamUtil.log("debug", "id=" + sellPdf.getId() + " status=" + sellPdf.getStatus()) ;
            if(sellPdf.getStatus().equals(ConsoleUtil.VEAM_SELL_PDF_STATUS_READY)){
                titleLabel.setTextColor(Color.BLACK) ;
                priceLabel.setTextColor(Color.BLACK) ;
            } else {
                thumbnailImageView.setBackgroundColor(Color.RED) ;

                if(sellPdf.getStatus().equals(ConsoleUtil.VEAM_SELL_PDF_STATUS_SUBMITTING)){
                    ...
                    statusBlackLabel.setText(sellPdf.getStatusText()) ;
                ...
                statusLabel.setText(sellPdf.getStatusText()) ;
                ...
                if(sellPdf.getStatusText().equals("Preparing")){
